...
const MessageItem = ({
  ...
  // Check if image URL is blurred/pixelated (policy violation)
  const isImageBlurred = (url) => {
    if (!url) return false;
    return url.includes('e_blur') || url.includes('e_pixelate');
  };
  ...
  const renderMessageContent = () => {
    ...
    if (message.message_type === 'image' && groupedImages && groupedImages.length > 0) {
      const total = groupedImages.length;
      const caption = message.content;
      const isAutoGenerated = caption.includes('Đã gửi') && caption.includes('hình ảnh');

      return (
        <div className="message-image-group">
          <div className={`image-grid image-grid-${Math.min(total, 4)}`}>
            {groupedImages.slice(0, total > 4 ? 4 : total).map((img, idx) => {
              const blurred = isImageBlurred(img.file_url);

              if (blurred) {
                return (
                  <div key={img.message_id} className="grid-item">
                    <div className="blurred-image-notice">
                      <i className="fa-solid fa-exclamation-triangle"></i>
                      <p>Vi phạm chính sách nên ảnh bị làm mờ</p>
                    </div>
                    {total > 4 && idx === 3 && (
                      <div className="more-overlay">+{total - 4}</div>
                    )}
                  </div>
                );
              }
              ...
              return (
                <div key={img.message_id} className="grid-item">
                  <img src={img.file_url} alt={`Ảnh ${idx + 1}`} loading="lazy" />
                  {total > 4 && idx === 3 && (
                    <div className="more-overlay">+{total - 4}</div>
                  )}
                </div>
          ...
          {caption && !isAutoGenerated && (
            <div className="image-group-caption">{caption}</div>
          )}
          {isAutoGenerated && (
            <div className="image-group-caption auto-caption">
              Photos {caption}
            </div>
          )}
        </div>
